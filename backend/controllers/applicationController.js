...
const applyJob = async (req, res) => {
    ...
    if (!job) {
        throw BadRequestError('Job not found');
    }
    if (job.status !== 'open') {
        throw BadRequestError('This job is no longer accepting applications');
    }
    ...
    if (existingApplication) {
        throw BadRequestError('You have already applied for this job');
    }
...
const updateApplicationStatus = async (req, res) => {
    ...
    if (!job || job.employer.toString() !== req.user.userId) {
